...
Both exist to test one claim the rest of the paper cannot: that a window of joint
encoders determines the privileged base state *because the stance foot ties the
base to a fixed frame*. Every result elsewhere was measured with friction pinned
at ``(1.0, 1.0)`` on a plane, which is exactly the condition under which that tie
is strongest.

* **Friction** attacks whether the contact holds. A slipping foot is not a fixed
  frame, so the joint chain stops determining base *velocity*.
* **Slope** attacks what the contact is fixed *to*. Joint angles give base
  ...
  contains -- only the history can carry it, if anything can.

Nothing here edits :mod:`walk_env_cfg` or :mod:`walk_estimator_env_cfg`. Those two
files are hashed into ``TASK_IMPLEMENTATION["locomotion"]`` and into the friction
sweep's ``SOURCE_FILES``, so changing a byte of either would invalidate the digest
of every locomotion study already recorded. Everything below is a subclass.

Deliberately *not* varied here: base mass and external pushes. Commit ``a002570``
has both, and the docstring of ``EventCfg`` invites restoring them, but this study
changes the terrain and nothing else -- two variables at once would make a
degraded number unattributable.
"""
...
class FrictionEventCfg(EventCfg):
    ...
    The range is restored, not invented: ``EventCfg``'s own docstring says to
    "widen ``physics_material``'s ranges back to (0.3, 1.0)", and commit
    ``a002570`` is where they came from. Restitution stays at zero and the two
    reset terms are inherited untouched.
    ...
    effective contact coefficient -- the same convention
    ``eval_friction_robustness.py`` relies on when it sweeps friction at
    evaluation time.
    """
...
#:    changes the 495-D policy layout that ``schema.py`` hardcodes and
#:    ``test_jose.py`` asserts -- which would change the estimator interface the
#:    entire paper is built on. A slope is locally flat, so a blind policy trained
