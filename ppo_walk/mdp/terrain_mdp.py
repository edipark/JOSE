...
(``velocity_env_cfg.py:271``). We do not take it: the paper defines survival as
the base dropping below a task-specific height (``main.tex``, *Metrics*), and the
sloped block is only readable beside the flat one if the two measure the same
event. So the definition is kept and the *reference* is made terrain-relative,
using the same ray-cast ground height that
:func:`~isaaclab.envs.mdp.rewards.base_height_l2` already accepts a sensor for.

This module is deliberately separate from ``ppo_walk/mdp/rewards.py``. That file
is hashed into ``TASK_IMPLEMENTATION["locomotion"]`` and into the friction
sweep's ``SOURCE_FILES``; editing it would invalidate the digest of every
locomotion study already recorded.
"""
